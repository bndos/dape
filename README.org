...
+ VS Code =launch.json= compatibility for supported adapters
...
=Dape= also reads VS Code =.vscode/launch.json= files from the current project root by default. Launch configurations are exposed as generated =dape-configs= entries with a =launch-json-= prefix and are merged onto matching Dape adapter configurations by =type= and =request=. This means =launch.json= provides the debug arguments while Dape still provides the debug adapter command. JSONC comments, trailing commas, platform overrides, common VS Code variables such as =${workspaceFolder}= and =${file}=, and simple =preLaunchTask= entries from =.vscode/tasks.json= are supported.
...
+ =Dape= supports ~launch.json~ files for adapters that have matching entries in ~dape-configs~. Per-project Emacs-specific configuration can still use ~dir-locals~ and ~dape-command~.
...
+ Emacs Lisp functions and variables in ~dape-configs~ are resolved before starting a new session; VS Code variables in ~launch.json~ are expanded when the file is imported.
